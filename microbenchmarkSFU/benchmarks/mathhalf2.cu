...
#define FP_TYPE half_float::half
#define FP_DEV_TYPE half
#define FP_TYPE_REF double
#define LOOP 500
#define INC_DIFF __float2half2_rn(0.001)
#define HOST_FUNC asin
#define DEV_FUNC fast_h2asin
//#define PERFORMANCE
...
__global__ void Vec_add(FP_DEV_TYPE x[], FP_DEV_TYPE y[], FP_DEV_TYPE z[], int n) {
   ...
   half2 sink = __float2half2_rn(0.0);

   if (tid < n/2) {
      half2 temp = ((half2*)x)[tid];
...
//   cudaMemcpy(buffer, h_buffer, MAX_TEXTURE_SIZE*sizeof(float), cudaMemcpyHostToDevice); //copy data to texture
 ...
 printf("Elapsed time : %f ms\n" ,elapsedTime);
  ...
  double err = 0;
   for (i = 0; i < n; i++) {
     if(h_z_ref[i]!=0){
        double temp = fabs((h_z[i]-h_z_ref[i])/h_z_ref[i]);
        if (temp > 1.0) //cap rel error at 1.0
          temp = 1.0;
        err += temp;//fabs((h_z[i]-h_z_ref[i])/h_z_ref[i]);
/*
        if (isinf(err)){ //debug
          printf ("%f  %f \n", fabs((h_z[i]-h_z_ref[i])/h_z_ref[i]), h_z_ref[i]);
          break;
        }
*/
      }
